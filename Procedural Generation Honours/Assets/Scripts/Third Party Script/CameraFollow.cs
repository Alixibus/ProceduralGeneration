...
{
    //Script taken from https://www.youtube.com/watch?v=VyOGduSxyqk
    //Used with free licence
    Transform rootNode;
    Transform carCam;
    Transform car;
    ...
    public float cameraStickiness = 5.0f;
    ...
    void FixedUpdate()
    {
        ...
        if(car == null)
        {
            car = GameObject.FindGameObjectWithTag("CameraAimAt").GetComponent<Transform>();
            carPhysics = GameObject.FindGameObjectWithTag("GetawayVehicle").GetComponent<Rigidbody>();
        }

        if (car != null)
        {
            // Moves the camera to match the car's position.
            rootNode.position = Vector3.Lerp(rootNode.position, new Vector3(car.position.x, car.position.y + 0.03f, car.position.z), cameraStickiness * Time.fixedDeltaTime);
